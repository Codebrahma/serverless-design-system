...
const DocContent = ({ data: { allMarkdownRemark } }) => {
    ...
    <div style={{ fontSize: '16px' }}>
      {
        sortedNodes.map((item, index) => {
          return (item.node.frontmatter.path.split('/')[2]) === 'props' ? (
            <div key={index}>
              <h3>
                Props for {item.node.frontmatter.title}
              </h3>
...
export const query = graphql`
  query Docs($type: String!, $componentName: String!) {
    allMarkdownRemark(filter: { frontmatter: { component: { eq: $componentName }, type: { eq: $type } } }) {
      edges {
        node {
          id
          html
